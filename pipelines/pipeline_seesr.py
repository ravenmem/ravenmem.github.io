...
#                 if ram_encoder_hidden_states.ndim == 4:
#                     ram_encoder_hidden_states = torch.cat([ram_encoder_hidden_states, ram_encoder_hidden_states], dim=1)
#                 else:
#                     ram_encoder_hidden_states = torch.cat([ram_encoder_hidden_states, ram_encoder_hidden_states], dim=0)
...
#         output_confidence_map: bool = False,
#         args=None
#     ):
#         r"""
...
#         print('ram_encoder_hidden_states.shape', ram_encoder_hidden_states.shape)
...
#         elif start_point == 'lr': # LRE Strategy
#             vae_input = image * 2.0 - 1.0
#             lre_latents = self.vae.encode(vae_input.to(dtype=self.vae.dtype)).latent_dist.sample(generator)
#             lre_latents = lre_latents * self.vae.config.scaling_factor
#             lre_latents = lre_latents - lre_latents.mean(dim=[1, 2, 3], keepdim=True)

#             start_steps_tensor = torch.full((latents.shape[0],), start_steps, device=device, dtype=torch.long)
#             latents = self.scheduler.add_noise(lre_latents[0:1, ...], latents, start_steps_tensor)
...
#                     controlnet_latents = self.vae.encode(image).latent_dist.sample()
...
#                     )
#                     if i == 0:
#                         print(f"ControlNet Output Max: {down_block_res_samples[-1].max().item()}")
...
class StableDiffusionControlNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def prepare_metadata(self, batch_size, metadata, do_classifier_free_guidance, device, dtype,):
    ...
    def __call__(
        ...
        ### CUSTOM for sar2opt
        input_metadata = self.prepare_metadata(batch_size, metadata, do_classifier_free_guidance, device, prompt_embeds.dtype)
        
        # if input_metadata is not None:
        #     assert input_metadata.shape[-1] == self.unet.config.num_metadata
