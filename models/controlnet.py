...
class ControlNetModel(ModelMixin, ConfigMixin, FromOriginalControlnetMixin):
    ...
    def __init__(
        ...
        image_cross_attention_dim: int = None,
        ...
        if use_metadata:
            # controlnet_cond 채널 수 = block_out_channels[0]
            cond_channels = block_out_channels[0]
            # metadata 벡터를 (gamma, beta) 로 바꾸는 작은 MLP
            self.metadata_film = nn.Sequential(
                nn.Linear(num_metadata, cond_channels * 2),
                nn.SiLU(),
    ...
    def forward(
        ...
        #             cond_metadata = self.time_proj(cond_metadata.view(-1)).view(md_bsz, self.num_metadata, num_cond, -1)  # (N, num_md, D)
        ...
        if self.metadata_film is not None and metadata is not None:
            # metadata: (B, num_metadata)
            md = metadata.to(dtype=sample.dtype)

            # (B, 2*C)
            film_params = self.metadata_film(md)
            cond_channels = controlnet_cond.shape[1]

            gamma, beta = film_params.split(cond_channels, dim=1)  # 각 (B, C)

            # shape 맞춰서 broadcast: (B, C, 1, 1) -> (B, C, H', W')
            gamma = gamma.unsqueeze(-1).unsqueeze(-1)
            beta  = beta.unsqueeze(-1).unsqueeze(-1)

            controlnet_cond = (1 + gamma) * controlnet_cond + beta
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            if is_multiscale_latent and image_encoder_hidden_states is not None:
                if image_encoder_hidden_states.dim() == 4:
                    ...
                    # (혹시 블록 개수가 더 많을 경우를 대비해 min으로 인덱스 제한)
        ...
        if self.mid_block is not None:
            if is_multiscale_latent and image_encoder_hidden_states is not None and image_encoder_hidden_states.dim() == 4:
